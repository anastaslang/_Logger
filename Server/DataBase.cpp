...
void DataBase::openDataBase()
{
    ...
    if(&mysql == nullptr)
    {
        std::cout << "Дескриптор соединения с базой данных не создан" << std::endl;
        exit(1);
    }
    MYSQL *connect = mysql_real_connect(&mysql, "localhost", "root", "111", "testdb", 0, NULL, 0);
    if(connect == nullptr)
    {
        std::cout << "Нет подключения к базе данных  " << mysql_error(&mysql) << std::endl; 
        exit(1);       
    }    
}

void DataBase::createDataTable()
{
...
bool DataBase::addDataUser(const std::string &name, const std::string &login, const std::string &password)
{        
...
bool DataBase::enterDataUser(const std::string &login, const std::string &password)
{         
...
bool DataBase::addDataMessage(const std::string &from, const std::string &to, const std::string &text)
{
...
bool DataBase::addDataMessageAll(const std::string &from, const std::string &text)
{
...
std::string DataBase::showDataUsersLogin(const std::string &login)
{
...
bool DataBase::correctDataEnterPerson(const std::string& login_to)
{
...
std::string DataBase::showDataMessage(const std::string &login)
{        
...
std::string DataBase::showDataMessageAll()
{
